...
from views.alert_center import AlertCenterWidget
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.tab_widget.setObjectName("content_tabs")
        self.tab_widget.setTabsClosable(False)
        self.tab_widget.setMovable(True)
        self.tab_widget.setDocumentMode(True)

        # 添加业财看板
        self.dashboard = DashboardWidget(self)  # 传递self作为父对象，以便访问token
        self.tab_widget.addTab(self.dashboard, "业财看板")

        # 添加预警中心
        self.alert_center = AlertCenterWidget(self)  # 传递self作为父对象，以便访问token
        self.tab_widget.addTab(self.alert_center, "预警中心")

        # 添加内容页面到标签页
        self.business_view = BusinessEventView(self.token, self.user_data)
        self.tab_widget.addTab(self.business_view, "业务数据")

        if self.user_data['role'] in ['管理员', '财务人员']:
            self.finance_view = FinancialRecordView(self.token, self.user_data)
            self.tab_widget.addTab(self.finance_view, "财务记录")

        if self.user_data['role'] in ['管理员', '业务人员', '部门主管']:
            self.approval_view = ApprovalView(self.token, self.user_data)
            self.tab_widget.addTab(self.approval_view, "审批流程")

        if self.user_data['role'] in ['管理员', '财务人员']:
            self.budget_view = BudgetView(self.token, self.user_data)
            self.tab_widget.addTab(self.budget_view, "预算分析")
    ...
    def change_page(self, index, button, title):
        ...
        # 切换到相应的标签页
        if index == 0:
            self.tab_widget.setCurrentWidget(self.business_view)
        elif index == 1 and hasattr(self, 'finance_view'):
            self.tab_widget.setCurrentWidget(self.finance_view)
        elif index == 2 and hasattr(self, 'approval_view'):
            self.tab_widget.setCurrentWidget(self.approval_view)
        elif index == 3 and hasattr(self, 'budget_view'):
            self.tab_widget.setCurrentWidget(self.budget_view)
