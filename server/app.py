...
async def create_customer(customer: Customer, current_user = Depends(get_current_user)):
    ...
    try:
        # 检查客户名称是否已存在
        existing = cursor.execute("SELECT id FROM customers WHERE name = ?", (customer.name,)).fetchone()
        if existing:
            raise HTTPException(status_code=400, detail="客户名称已存在")

        # 插入新客户
        cursor.execute("""
            INSERT INTO customers (name, code, contact_person, contact_phone, contact_email, address, email)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (customer.name, customer.code, customer.contact_person, customer.contact_phone,
              customer.contact_email, customer.address, customer.email))

        customer_id = cursor.lastrowid
        conn.commit()

        return {"id": customer_id, "message": "客户创建成功"}
...
async def get_all_financial_transactions(current_user = Depends(get_current_user)):
    """获取所有财务交易记录"""
    conn = get_db_connection()
...
@app.get("/business_events/{event_id}/financial_transactions")
async def get_business_financial_transactions(event_id: int, current_user = Depends(get_current_user)):
    """获取业务事件关联的所有财务交易记录"""
    conn = get_db_connection()
    transactions = conn.execute("""
...
@app.post("/financial_transactions")
async def create_financial_transaction(transaction: FinancialTransaction, current_user = Depends(get_current_user)):
    """创建财务交易记录"""
    # 检查用户是否有财务角色
    ...
    try:
        # 检查业务事件是否存在
        event = cursor.execute("SELECT * FROM business_events WHERE id = ?",
                             (transaction.business_event_id,)).fetchone()
        if not event:
            conn.close()
            raise HTTPException(status_code=404, detail="业务事件不存在")
        ...
        # 检查业务事件状态是否允许添加财务记录
        if event_dict["status"] not in ["已审批", "执行中"]:
            conn.close()
            raise HTTPException(status_code=400, detail="只有已审批或执行中的业务事件才能添加财务记录")

        # 如果状态是"已审批"，先更新为"执行中"
        if event_dict["status"] == "已审批":
            cursor.execute("UPDATE business_events SET status = '执行中' WHERE id = ?",
                         (transaction.business_event_id,))

            # 添加状态历史记录
            cursor.execute("""
                INSERT INTO status_history (business_event_id, status, operator, remarks)
                VALUES (?, ?, ?, ?)
            """, (transaction.business_event_id, "执行中", current_user["username"], "开始执行业务事件"))

        # 插入财务交易记录
        cursor.execute("""
            INSERT INTO financial_transactions (
                business_event_id, account_code, account_name, amount, direction,
                transaction_date, fiscal_year, fiscal_period, description, created_by
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            transaction.business_event_id, transaction.account_code, transaction.account_name,
            transaction.amount, transaction.direction, transaction.transaction_date,
            transaction.fiscal_year, transaction.fiscal_period, transaction.description,
        ...
        # 获取创建的记录
        result = cursor.execute("SELECT * FROM financial_transactions WHERE id = ?",
        ...
        conn.rollback()
        conn.close()
        raise HTTPException(status_code=500, detail=f"创建财务交易记录失败: {str(e)}")
...
async def complete_business_event(event_id: int, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO status_history (business_event_id, status, operator, remarks)
            VALUES (?, ?, ?, ?)
        """, (event_id, "已完成", current_user["username"], "业务事件已完成"))
        ...
        conn.rollback()
        conn.close()
        raise HTTPException(status_code=500, detail=f"完成业务事件失败: {str(e)}")
